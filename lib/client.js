...
`;
    ...
    function QuotaPanel({ quota, state, busy, t }) {
      ...
      return React.createElement(React.Fragment, null,
        ...
        state === "error" && React.createElement("div", { className: "dsha-error", role: "status" }, t("quotaUnavailable")),
    ...
    function AntigravitySettings({ ctx }) {
      ...
      const invalidateReads = useCallback(() => {
        statusSeq.current++; quotaSeq.current++; quotaRequestSeq.current.clear(); modelSeq.current++;
      ...
      const applyStatus = useCallback((value) => {
        ...
        if (changed) { quotaSeq.current++; quotaRequestSeq.current.clear(); modelSeq.current++; setModelConfig(undefined); }
        stateRef.current = value;
        setStatus({ ...value, loading: false });
        if (value.quota && value.quota.accountId) {
           setQuotaByAccount((current) => current[value.quota.accountId]?.fetchedAt > value.quota.fetchedAt ? current : ({ ...current, [value.quota.accountId]: value.quota }));
           setQuotaStateByAccount((current) => ({ ...current, [value.quota.accountId]: "loaded" }));
         }
      ...
      const refreshQuota = useCallback(async (requestedAccountId) => {
        ...
        try {
          ...
          if (live.current && quotaRequestSeq.current.get(accountId) === seq && revision === stateRef.current.revision && value.accountId === accountId) {
            setQuotaByAccount((current) => ({ ...current, [accountId]: value }));
            setQuotaStateByAccount((current) => ({ ...current, [accountId]: "loaded" }));
            if (accountId === stateRef.current.activeAccountId && value.models) setModelConfig(value.models);
          }
        } catch (_) {
          if (live.current && quotaRequestSeq.current.get(accountId) === seq) setQuotaStateByAccount((current) => ({ ...current, [accountId]: "error" }));
        }
      }, [request]);
      ...
      useEffect(() => {
        ...
        if (accounts.length > 0) void Promise.all(accounts.map((account) => refreshQuota(account.id)));
      }, [accountIds, status.revision, refreshQuota]);
      ...
      useEffect(() => {
        ...
        const tick = async () => {
          ...
          try {
            await refreshStatus();
            if (!disposed && !mutationLock.current) {
              await Promise.all((stateRef.current.accounts || []).map((account) => refreshQuota(account.id)));
            }
          } catch (_) {
      ...
      const mutate = useCallback(async (path, payload) => {
        ...
        invalidateReads(); setBusy(true); setError(""); setQuotaByAccount({}); setQuotaStateByAccount({});
        let succeeded = false;
        try {
          ...
          if (live.current) { setBusy(false); if (succeeded) void refreshAll(); }
        }
        return succeeded;
