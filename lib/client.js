...
`;
    ...
    function AntigravityQuotaIndicator({ directory, t: propT, ctx }) {
      ...
      useEffect(() => {
        ...
        void fetchQuota();
        const timer = window.setInterval(() => { void fetchQuota(); }, USAGE_POLL_INTERVAL_MS);
        return () => {
          disposed = true;
          window.clearInterval(timer);
          controller.abort();
      ...
      const buckets = matchBucketsForModel(quotaData, modelId);
