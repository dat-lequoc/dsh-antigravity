        ...
        React.createElement("section", { className: "dsha-card dsha-account-card" },
          ...
          React.createElement("div", { className: "dsha-account-list", role: "group", "aria-label": tr("accounts") }, (status.accounts || []).map((account) => {
            ...
            return React.createElement("div", { key: account.id, className: "dsha-account-row" },
              ...
              active && React.createElement("div", { className: "dsha-account-quota" },
                !quota && React.createElement("div", { className: "dsha-empty dsha-quota-empty" }, busy ? tr("fetchingQuota") : tr("noQuotaDesc")),
                (quotaGroups.length > 0 || fallbackModelRows.length > 0) && React.createElement("div", { className: "dsha-quota-title" }, tr("quota")),
                quotaGroups.map((group, index) => {
                  const isCyan = /claude|gpt|3p|openai|anthropic/i.test(`${group.displayName || ""} ${group.description || ""}`);
                  return React.createElement(QuotaGroup, { key: group.displayName || index, group, accent: isCyan ? "cyan" : "green", t: tr });
                }),
                quotaGroups.length === 0 && fallbackModelRows.map((row, index) => {
                  const isCyan = /claude|gpt|3p|openai|anthropic/i.test(`${row.id || ""} ${row.label || ""}`);
                  return React.createElement(QuotaRow, { key: row.id || row.label || index, row, accent: isCyan ? "cyan" : "green", t: tr });
                }),
                quota && React.createElement("div", { className: "dsha-note dsha-quota-updated" }, tr("updatedAt", { time: new Date(quota.fetchedAt).toLocaleString() })),
              ),
            );
          })),
