...
`;
    ...
    function isAntigravityModel(state) {
      const current = state && state.current;
      return Boolean(
        state && state.status === "ready" &&
        current && current.provider === "antigravity" &&
        typeof current.model === "string"
      );
    }

    function boundedQuotaPercent(percent) {
    ...
    function bucketShortLabel(bucket) {
      const text = `${bucket.bucketId || bucket.id || ""} ${bucket.displayName || bucket.label || ""} ${bucket.window || ""}`.toLowerCase();
      if (text.includes("5h") || text.includes("5-hour") || text.includes("five hour") || text.includes("5 hour")) {
        return "5h";
      }
      if (text.includes("week") || text.includes("7d") || text.includes("weekly") || text.includes("7-day") || text.includes("7 day")) {
        return "7d";
      }
      if (text.includes("day") || text.includes("24h") || text.includes("daily") || text.includes("24-hour") || text.includes("24 hour")) {
        return "24h";
      }
      return String(bucket.displayName || bucket.label || bucket.bucketId || bucket.id || "quota").slice(0, 3);
    }

    function matchBucketsForModel(quota, modelId) {
      if (!quota) return [];
      const groups = quotaGroupsOf(quota);
      const cleanId = String(modelId || "").replace(/^antigravity\//, "").toLowerCase();
      const isGemini = cleanId.includes("gemini");
      const is3P = /claude|gpt|3p|openai|anthropic/i.test(cleanId);

      let matchedGroup = null;
      if (groups.length > 0) {
        if (isGemini) {
          matchedGroup = groups.find((g) => /gemini/i.test(`${g.displayName || ""} ${g.description || ""}`));
        } else if (is3P) {
          matchedGroup = groups.find((g) => /claude|gpt|3p|openai|anthropic/i.test(`${g.displayName || ""} ${g.description || ""}`));
        }
        if (!matchedGroup) {
          matchedGroup = groups.find((g) => (g.buckets || []).some((b) => {
            const bId = String(b.bucketId || b.id || b.displayName || "").toLowerCase();
            return cleanId.includes(bId) || bId.includes(cleanId);
          }));
        }
      ...
      if (matchedGroup && Array.isArray(matchedGroup.buckets) && matchedGroup.buckets.length > 0) {
        return matchedGroup.buckets;
      }

      const rows = quotaRowsOf(quota, groups);
      if (rows.length > 0) {
        const matchingRow = rows.find((r) => {
          const rId = String(r.id || r.label || "").toLowerCase();
          return cleanId.includes(rId) || rId.includes(cleanId);
        }) || rows[0];
        if (matchingRow) {
          return [matchingRow];
        }
    ...
    function AntigravityQuotaIndicator({ directory, t: propT, ctx }) {
      const tr = propT || (ctx ? createTranslator(ctx) : (k) => k);
      const subscribe = useCallback((onStoreChange) => {
        if (directory && typeof directory.subscribe === "function") {
          return directory.subscribe(onStoreChange);
        }
        return () => {};
      }, [directory]);

      const getSnapshot = useCallback(() => {
        if (directory && typeof directory.getSnapshot === "function") {
      ...
      const directoryState = useSyncExternalStore
        ...
        : (directory && typeof directory.getSnapshot === "function" ? directory.getSnapshot() : undefined);

      const eligible = isAntigravityModel(directoryState);
      const [quotaData, setQuotaData] = useState(null);
      const [isHovered, setIsHovered] = useState(false);
      const [isFocused, setIsFocused] = useState(false);
      const generatedTooltipId = useId ? useId() : "dsha-quota-tip";
      ...
      useEffect(() => {
        ...
        const fetchQuota = async (force = false) => {
          ...
          try {
            ...
            if (quotaRes.ok) {
              ...
              if (qJson && qJson.ok && qJson.value && !disposed && !controller.signal.aborted) {
                lastFetch = Date.now();
                setQuotaData(qJson.value);
                return;
              }
            }
            ...
            if (res.ok) {
              ...
              const val = json && json.ok ? json.value : null;
              if (val && val.quota && !disposed && !controller.signal.aborted) {
                lastFetch = Date.now();
                setQuotaData(val.quota);
              }
            }
          } catch (_) {
            // Transient fetch failure; leave current state intact
          } finally {
            inFlight = false;
          }
        };
        ...
        const timer = window.setInterval(() => { void fetchQuota(true); }, USAGE_POLL_INTERVAL_MS);
        const onFocus = () => { void fetchQuota(false); };
        const onVisibility = () => {
          if (document.visibilityState !== "hidden") void fetchQuota(false);
        };
      ...
      if (!eligible || !quotaData) return null;

      const modelId = directoryState && directoryState.current ? directoryState.current.model : "";
      const rawBuckets = matchBucketsForModel(quotaData, modelId);
      if (!rawBuckets || rawBuckets.length === 0) return null;

      const bucketOrder = { "5h": 1, "24h": 2, "7d": 3 };
      const buckets = [...rawBuckets].sort((a, b) => {
        const oA = bucketOrder[bucketShortLabel(a)] || 10;
        const oB = bucketOrder[bucketShortLabel(b)] || 10;
        return oA - oB;
      });

      const items = buckets.map((bucket, index) => {
        const shortLabel = bucketShortLabel(bucket);
        const disabled = Boolean(bucket && bucket.disabled === true);
        const percent = percentOf(bucket);
        const knownPercent = typeof percent === "number" && Number.isFinite(percent);
        const progressPercent = knownPercent ? boundedQuotaPercent(percent) : 0;
        const color = quotaProgressColor(progressPercent);
        let summaryPart = "";
        if (disabled) {
          summaryPart = `${shortLabel}: ${tr("notEnforced")}${bucket.description ? ` (${bucket.description})` : ""}`;
        } else {
          const remText = tr("remainingLabel", { percent: knownPercent ? percentText(percent) : "?" });
          const rText = resetText(bucket, tr);
          summaryPart = `${shortLabel}: ${remText}${bucket.resetTime || bucket.resetLabel ? `, ${rText}` : ""}`;
        }
        return {
          id: bucket.id || bucket.bucketId || index,
          shortLabel,
          disabled,
      ...
      const summary = items.map((it) => it.summaryPart).join("; ");
      const tooltipVisible = isHovered || isFocused;

      return React.createElement("span", {
        role: "status",
        "data-antigravity-quota": items.map((it) => it.shortLabel).join(","),
        "aria-label": summary,
        "aria-describedby": tooltipVisible ? generatedTooltipId : undefined,
        tabIndex: 0,
        ...
        items.map((it) => React.createElement("span", {
          ...
          React.createElement("span", {
            "data-antigravity-quota-track": it.shortLabel,
            style: {
              display: "block",
              width: `${QUOTA_PROGRESS_WIDTH_PX}px`,
              height: `${QUOTA_PROGRESS_TRACK_HEIGHT_PX}px`,
              borderRadius: "999px",
              backgroundColor: "var(--dsw-alias-border-l2, rgba(0, 0, 0, 0.08))",
              overflow: "hidden",
            },
          },
            !it.disabled ? React.createElement("span", {
              "data-antigravity-quota-progress": it.shortLabel,
              "data-antigravity-quota-color": it.color.name,
              style: {
                display: "block",
                width: `${it.percent}%`,
                height: "100%",
                borderRadius: "inherit",
                backgroundColor: it.color.value,
        ...
        tooltipVisible ? React.createElement("span", {
          ...
          role: "tooltip",
          "data-antigravity-quota-tooltip": "",
          style: {
            position: "absolute",
            bottom: "calc(100% + 6px)",
            left: "50%",
            ...
            boxShadow: "var(--dsw-shadow-lv2, 0 4px 12px rgba(0, 0, 0, 0.12))",
            fontSize: "12px",
            lineHeight: "18px",
          },
        }, summary) : null,
    ...
    return {
      ...
      apply(ctx) {
        ...
        ctx.inject(["slots", "modelDirectories"], (scope) => {
          const tr = createTranslator(scope || ctx);
          ...
          }, (props) => React.createElement(AntigravityQuotaIndicator, { ...props, t: tr, ctx })));
        });
      },
    };
  },
