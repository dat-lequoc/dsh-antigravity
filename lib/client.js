...
.dsha-account-card>.dsha-account-list>.dsha-quota-group{margin:0;padding:10px 12px}
.dsha-account-card>.dsha-quota-title,.dsha-account-card>.dsha-quota-group,.dsha-account-card>.dsha-quota-empty,.dsha-account-card>.dsha-quota-updated{order:2}
...
.dsha-quota-title{display:flex;align-items:baseline;gap:10px}.dsha-quota-account{font-size:12px;font-weight:500;color:#8b93a1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.dsha-account-list .dsha-actions{justify-content:flex-start;margin-top:8px}
.dsha-account-list .dsha-note{margin-top:5px}
.dsha-account-row{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:14px 16px;border:1px solid #e7ebf1;border-radius:12px;background:#fff;min-width:0}
.dsha-account-main{min-width:0;flex:1 1 auto}.dsha-account-name{display:flex;align-items:center;gap:8px;min-width:0;font-size:14px;font-weight:750;color:#111827}.dsha-account-name:first-letter{font-weight:800}.dsha-account-email,.dsha-account-route,.dsha-account-health{margin-top:4px;color:#8b93a1;font-size:12px;line-height:17px;overflow-wrap:anywhere}.dsha-account-health.ready{color:#059669}.dsha-account-health.needs{color:#b45309}.dsha-account-actions{display:flex;align-items:center;justify-content:flex-end;gap:6px;flex:0 0 auto;flex-wrap:wrap}.dsha-btn-danger{color:#b42318}.dsha-account-card>.dsha-account-list{display:flex;flex-direction:column;gap:8px}.dsha-account-card>.dsha-account-list>.dsha-account-row{margin:0}
@media (max-width:640px){.dsha-account-row{align-items:flex-start;flex-direction:column}.dsha-account-actions{width:100%;justify-content:flex-start}.dsha-account-actions .dsha-btn{flex:1 1 auto}}
    ...
    function AntigravitySettings({ ctx }) {
      ...
      return React.createElement("div", { className: "dsha-wrap" },
        ...
        React.createElement("section", { className: "dsha-card dsha-account-card" },
          ...
          React.createElement("div", { className: "dsha-account-list", role: "group", "aria-label": tr("accounts") }, (status.accounts || []).map((account) => {
            ...
            return React.createElement("div", { key: account.id, className: "dsha-account-row" },
              React.createElement("div", { className: "dsha-account-main" },
                ...
                account.label && account.email ? React.createElement("div", { className: "dsha-account-email" }, account.email) : null,
                React.createElement("div", { className: `dsha-account-health ${account.authState === "ready" ? "ready" : "needs"}` }, account.authState === "ready" && Number.isFinite(account.expires) ? tr("expiresAt", { time: new Date(account.expires).toLocaleString() }) : tr("expired")),
                React.createElement("div", { className: "dsha-account-route" }, proxy.enabled ? `${tr("proxy")}: ${proxy.url || ""} · ${tr(proxy.scope === "all-account" ? "allAccount" : "authOnly")}` : tr("proxyOff")),
              ),
              React.createElement("div", { className: "dsha-account-actions" },
                React.createElement("button", { className: "dsha-btn dsha-btn-primary", disabled: active || busy || modelBusy, "aria-pressed": active, onClick: () => { closeEditor(); void mutate("/accounts/activate", { accountId: account.id, expectedRevision: stateRef.current.revision }); } }, active ? tr("active") : tr("useAccount")),
                ...
                React.createElement("button", { className: "dsha-btn dsha-btn-danger", disabled: busy || modelBusy, onClick: (event) => openEditor("remove", { ...account, active }, event) }, tr("remove"))),
            );
          })),
          ...
          status.authenticated && (quota || busy) && React.createElement("div", { className: "dsha-quota-title" },
            tr("quota"), React.createElement("span", { className: "dsha-quota-account" }, status.email || "")),
          status.authenticated && !quota && React.createElement("div", { className: "dsha-empty dsha-quota-empty" }, busy ? tr("fetchingQuota") : tr("noQuotaDesc")),
          status.authenticated && quotaGroups.length === 0 && !fallbackModelRows.length && quota && React.createElement("div", { className: "dsha-empty dsha-quota-empty" }, tr("noQuotaDesc")),
          status.authenticated && (quotaGroups.length > 0 || fallbackModelRows.length > 0) && !quota && React.createElement("div", { className: "dsha-empty dsha-quota-empty" }, tr("fetchingQuota")),
          ...
          quota && React.createElement("div", { className: "dsha-note dsha-quota-updated" },
