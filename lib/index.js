...
async function fetchCapturedQuota(store, modelSettings) {
  ...
  // This endpoint is project-scoped. An empty body can return a 4xx or an
  // empty summary, which made the UI report that quota was unavailable.
  const summary = await postJson("/v1internal:retrieveUserQuotaSummary", token, { project: projectId });
