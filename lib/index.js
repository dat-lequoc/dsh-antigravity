...
function getFallbackRuntimeCandidates(runtimeModel, effort) {
  const fallbacks = [];
  if (runtimeModel.startsWith("gemini-3.8-flash") || runtimeModel === "gemini-3.8-flash-tiered") {
    fallbacks.push(getAntigravityRequestModelId("gemini-3.6-flash", effort) || "gemini-3.6-flash-high");
    fallbacks.push("gemini-3.6-flash-medium");
    fallbacks.push(getAntigravityRequestModelId("gemini-3.1-pro", effort) || "gemini-pro-agent");
  } else if (runtimeModel === "gemini-3.7-flash-tiered" || runtimeModel.startsWith("gemini-3.7-flash")) {
    fallbacks.push(getAntigravityRequestModelId("gemini-3.6-flash", effort) || "gemini-3.6-flash-high");
    fallbacks.push("gemini-3.6-flash-medium");
    fallbacks.push(getAntigravityRequestModelId("gemini-3.1-pro", effort) || "gemini-pro-agent");
  } else if (runtimeModel.startsWith("gemini-3.6-flash")) {
    fallbacks.push("gemini-3.5-flash-low");
    fallbacks.push(getAntigravityRequestModelId("gemini-3.1-pro", effort) || "gemini-pro-agent");
  } else if (runtimeModel.startsWith("claude-opus-4-6")) {
    fallbacks.push("claude-sonnet-4-6-thinking");
  } else if (runtimeModel.startsWith("claude-sonnet-4-6")) {
    fallbacks.push(getAntigravityRequestModelId("gemini-3.6-flash", effort) || "gemini-3.6-flash-high");
  }
  return fallbacks.filter((id) => id && id !== runtimeModel);
}

function getFallbackRuntimeModel(runtimeModel, effort) {
  const candidates = getFallbackRuntimeCandidates(runtimeModel, effort);
  return candidates[0];
}
...
function* emitTransientNotice(message, state) {
  const index = state.blocks.length;
  const block = { type: "reasoning", text: message + "\n\n" };
  state.blocks.push(block);
  state.replayBlocks.push({ type: "reasoning" });
...
async function resolveRuntimeCandidates(token, projectId, model, effort) {
  ...
  for (const fallback of getFallbackRuntimeCandidates(initialRuntimeModel, effort)) {
    if (!runtimeCandidates.includes(fallback)) runtimeCandidates.push(fallback);
  }
...
async function* requestAntigravityChunks(options, model, signal, store, attachments) {
  ...
  attemptLoop: for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (attempt > 0) {
      const delay = Math.min(1000 * 2 ** (attempt - 1) + Math.floor(Math.random() * 500), 8000);
      diagnostics.retries = (diagnostics.retries || 0) + 1;
      const waitNotice = `[Antigravity Notice: Retrying request (attempt ${attempt + 1}/${maxAttempts}) in ${Math.round(delay / 1000)}s after transient capacity/service error...]`;
    ...
    for (let candidateIndex = 0; candidateIndex < runtimeCandidates.length; candidateIndex++) {
      ...
      // Check if error is a capacity / transient error that warrants switching to a fallback model candidate
      const isCapacityError =
        response?.status === 503 ||
        /capacity/i.test(lastText) ||
        (response?.status === 429 && !/Individual quota reached/i.test(lastText));
      const isNotFoundOrTransient = [404, 500, 502, 504].includes(response?.status);

      if ((isCapacityError || isNotFoundOrTransient) && candidateIndex + 1 < runtimeCandidates.length) {
        const nextModel = runtimeCandidates[candidateIndex + 1];
        diagnostics.fallbackModelSwitches = (diagnostics.fallbackModelSwitches || 0) + 1;
        const friendly = friendlyAntigravityError(response?.status, lastText);
        const notice = `[Antigravity Notice: Model ${runtimeModel} reported capacity issue (${response?.status || 503}: ${friendly}). Switching to fallback model ${nextModel}...]`;
        yield* emitTransientNotice(notice, state);
        continue;
      }
    }
  }
...
export class AntigravitySearchProvider {
  ...
  available() {
    try {
      return existsSync(this.#store.path());
    } catch {
      return false;
    }
  }
  ...
  async search(request, signal) {
    ...
    const { token, projectId } = await ensureApiKey(this.#store);
...
function registerWebSearch(webCtx, store) {
  if (!webCtx?.web?.registerSearchProvider) return;

  const provider = new AntigravitySearchProvider(store, SEARCH_PROVIDER_ID);
  try {
    webCtx.web.registerSearchProvider(provider);
  } catch (err) {
    setLastError(err);
  }
  ...
  try {
    ...
    webCtx.web.registerSearchProvider(alias);
  } catch {
