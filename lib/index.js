...
export function parseQuotaSummary(data) {
  // The public endpoint has returned both the direct response shape and a
  // wrapped RPC shape over time (`{response:{groups:...}}`). Keep the UI
  // independent of that transport detail.
  ...
  const groups = [];
  const rawGroups = Array.isArray(summary.groups)
    ? summary.groups
...
async function fetchCapturedQuota(store, modelSettings) {
  ...
  // retrieveUserQuotaSummary is project-scoped. Sending `{}` makes the
  // endpoint return an empty/unauthorized summary, which looked like a UI
  // rendering bug because the request failed before any quota rows existed.
