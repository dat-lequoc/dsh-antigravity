...
function summary(record, activeId) {
  return { id: record.id, email: record.email, label: record.label, createdAt: record.createdAt, updatedAt: record.updatedAt, credentialRevision: record.credentialRevision, active: record.id === activeId, authState: record.credentials.expires > Date.now() ? 'ready' : 'refresh-required', proxy: copy(record.proxy) };
}
