...
export async function createOAuthFlow({ proxy, signal: parentSignal, discoverProject } = {}) {
  ...
  let accepted = false;
  function acceptCallback(raw) {
    let url;
    try { url = new URL(raw, callback); } catch { throw fail("Invalid callback URL."); }
    if (url.pathname !== "/oauth-callback" || url.searchParams.get("state") !== state) throw fail("Invalid OAuth callback state.");
    if (accepted) throw fail("Callback already accepted.", "CALLBACK_ALREADY_ACCEPTED");
    if (url.searchParams.has("error")) { accepted = true; rejectCode(fail("Google sign-in was not completed.")); return; }
    const code = url.searchParams.get("code");
    if (!code) throw fail("Missing authorization code.");
  ...
  const server = createServer((request, response) => {
    ...
    try { acceptCallback(url.href); send(200, "Authorization received. Return to DSH."); }
    catch (error) { send(error.code === "CALLBACK_ALREADY_ACCEPTED" ? 409 : 400, error.message); }
  });
  ...
  void result.catch(() => {});
  return { flowId, callbackUrl: callback, acceptCallback, authUrl: `https://accounts.google.com/o/oauth2/v2/auth?${params}`, result, cancel: () => controller.abort() };
}
